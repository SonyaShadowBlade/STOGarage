...
-- Диагностика клиентской переписки. Временная безопасная RPC:
-- позволяет проверить auth.uid() и фактическое состояние БД независимо от
-- developer_get_my_conversation(). После диагностики функцию можно удалить.
create or replace function public.developer_debug_my_conversation()
returns jsonb
language plpgsql
stable
security definer
...
begin
    ...
    if found_conversation_id is not null then
        ...
        into message_count
        from public.developer_messages dm
        where dm.conversation_id = found_conversation_id;

        select r.last_read_at
        into read_at
        from public.developer_conversation_reads r
        where r.conversation_id = found_conversation_id
    ...
    return jsonb_build_object(
        'marker', 'developer_debug_my_conversation_v1',
        'auth_uid', current_user_id,
        'conversation_exists', found_conversation_id is not null,
...
revoke all on function public.developer_debug_my_conversation() from public;
grant execute on function public.developer_debug_my_conversation() to authenticated;
